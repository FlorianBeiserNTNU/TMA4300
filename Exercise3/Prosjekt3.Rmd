...
```{r}
u.df <- read.table("https://www.math.ntnu.no/emner/TMA4300/2020v/Exercise/ex3-additionalFiles/u.txt",header=F)
z.df <- read.table("https://www.math.ntnu.no/emner/TMA4300/2020v/Exercise/ex3-additionalFiles/z.txt",header=F)
u = u.df$V1
z = z.df$V1
```
...
EM.algorihtm <- function(lam0, lam1){
  ...
  while(diff>1e-5){
    ...
    sum1 <- 0
    #for(k in 1:n){
      #sum0 <- sum0+u[k]*z[k]+(1-u[k])*(1/lam0i-z[k]/(exp(lam0i*z[k])-1))
      #sum1 <- sum1+((1-u[k])*z[k]+u[k]*(1/lam1i-z[k]/(exp(lam1i*z[k])-1)))
      #ifelse( u[k]==1, sum0 = sum0 + u[k]*z[k],  )
    #}
    ...
    diff <- norm(as.matrix(lamvec[1]-lam0i, lamvec[2]-lam1i))
