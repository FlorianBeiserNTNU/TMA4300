...
Using the uniform distribution over $(0,1)$ as proposal with density $g(\theta)=1$, the constant $c: \log f(\theta|y)\leq c \log g(\theta) = c \cdot 1$ is found by differentiating  $\frac{d}{d\theta}\log f(\theta|y)$ and equalizing $0=-(y_1+y_2+y_3+y_4)\theta^2 + (y_1-2y_2-2y_3-y_4)\theta + 2 \theta_4 = -197~\theta^2 + 15 ~\theta + 68$ which has a root at $0.626821...$.
...
c = posteriorLogScale(argmax_theta)
...
posteriorMonteCarloIntegration <- function(n){
...
posteriorMCMean = posteriorMonteCarloIntegration(sample_size)
...
curve(7.5/max(posterior(x))*posterior(x), from=0.0, to=1.0, add=TRUE)


# Comparison to numerical integration
posteriorNIMean = integrate(posterior, 0, 1)
print(paste("The NI estimate for the posterior mean is", posteriorNIMean))

```

In Figure \ref{fig:posterior_hist} we depict the histogram for the posterior distribution generated by rejection sampling with a unifrom proposal. We see that the sampled density has to little weight in around the mode compared to the analytical distribution. 

The numerical integration fails on the original problem scale. 
...
We observe that the practical implementation requires significantly less random number generations than the theory provides. 
...
The classical trick of importance sampling $\int{ \theta f(y|\theta)f_{1,5}(\theta) d \theta} = \int{ \theta f(y|\theta)f_{1,1}(\theta) \frac{f_{1,5}(\theta)}{f_{1,1}(\theta)}$ allows to reuse the samples from before, when we introduce the weights $w_i=\frac{f_{1,5}(\theta)}{f_{1,1}(\theta)} = f_{1,5}(\theta)$ into calculations.
