...
fCoreLogScale <- function(alpha, x){
  ...
  ifelse( x > 0, log(x+1)^(alpha-1)*exp(-log(x+1)), 0 )
...
myGammaRatioOfUniformsLogScale <- function(alpha, n){
  ...
  target = rep(0,n)
  while (num_accepted_samples < n ){
    x1 = log(runif(1, x1_lower, x1_upper)+1)
    x2 = log(runif(1, x2_lower, x2_upper)+1)
    if (x1 <= sqrt(fCoreLogScale(alpha,x2/x1))){
      num_accepted_samples = num_accepted_samples + 1
      target[num_accepted_samples] = x2/x1
    }
    num_total_samples = num_total_samples + 1 
...
hist(exp(samples)-1, breaks=50, freq=FALSE)
curve(f(alpha,x), from=0.0, to=10.0, add=TRUE)
...
Hence, we consider the moment generating function of above's sum: ........................
