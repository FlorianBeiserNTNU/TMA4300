...
ns = c(1000,5000,10000,25000,50000,75000,100000,200000)
err_means = rep(0, length(ns))
err_covs  = rep(0, length(ns))
for (i in 1:length(ns)){
  samples = myMultivariateNormal(d, mu, Sigma, ns[i])
  mean = 1/ns[i] * rowSums(samples)
  err_means[i] = norm(mean-mu, type="2")
  cov = 1/(ns[i]-1)*(samples-mean)%*%t(samples-mean)
  err_covs[i] = norm(cov-Sigma, type = "2")
}

plot(ns, err_means, type="l", xlab="n", ylab="error in mean vector")

plot(ns, err_covs, type="l", xlab="n", ylab="error in covariance matrix")
